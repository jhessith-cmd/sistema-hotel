...
router.get('/habitaciones', async (_req,res,next)=>{try{res.json((await appScriptGet('habitaciones')).map(r=>({id:r.ID_Habitacion,hospedajeId:r.ID_Hospedaje||'HOS-0001',numero:r.Numero||r['Número']||r.Habitacion||r['Habitación'],tipo:r.Tipo||'Estándar',capacidad:n(r.Capacidad),precio:n(r.Precio),estado:String(r.Estado||'Disponible').trim(),piso:n(r.Piso),activo:String(r.Activo||'Sí').trim(),observaciones:r.Observaciones||'',enAlquiler:String(r.Estado||'').trim()==='En alquiler'})))}catch(e){next(e)}});
...
router.post('/habitaciones/:id/estado', allowRoles('Administrador','Recepcionista','Limpieza','Mantenimiento'), async(req,res,next)=>{try{const estado=String(req.body.estado||'').trim();if(!['Disponible','Limpieza','Mantenimiento','Fuera de servicio','En alquiler'].includes(estado))throw new Error('Estado no permitido');res.json(await appScriptPost('cambiarEstadoHabitacion',{ID_Habitacion:req.params.id,Estado:estado,Responsable:req.user.nombre||req.user.sub,Observaciones:req.body.observaciones||'',Usuario_Registro:req.user.sub}))}catch(e){next(e)}});
...
router.post('/reservas/:id/accion', writeHotel, async(req,res,next)=>{try{res.json(await appScriptPost('accionReserva',{ID_Reserva:req.params.id,Accion:req.body.accion,Monto:n(req.body.monto),Metodo_Pago:req.body.metodoPago||'',Fecha:req.body.fecha||'',Hora:req.body.hora||'',Observaciones:req.body.observaciones||'',Fecha_Entrada:req.body.entrada||'',Fecha_Salida:req.body.salida||'',Total:req.body.total===''?undefined:n(req.body.total),ID_Habitacion:req.body.habitacionId||'',Cantidad_Personas:req.body.personas? n(req.body.personas):undefined,Numero_Comprobante:req.body.comprobante||'',Nombre_Huesped:req.body.nombreHuesped||'',Telefono:req.body.telefono||'',Fecha_Ingreso_Real:req.body.fechaIngresoReal||'',Hora_Ingreso:req.body.horaIngreso||'',Fecha_Salida_Real:req.body.fechaSalidaReal||'',Hora_Salida:req.body.horaSalida||'',Usuario_Registro:req.user.sub}))}catch(e){next(e)}});
...
router.get('/pagos', async (_req,res,next)=>{try{res.json((await appScriptGet('pagos')).map(p=>({id:p.ID_Pago,reservaId:p.ID_Reserva,fecha:p.Fecha_Pago,monto:n(p.Monto),metodo:p.Metodo_Pago,concepto:p.Concepto,estado:p.Estado,comprobante:p.Numero_Comprobante||'',observaciones:p.Observaciones||''}))) }catch(e){next(e)}});


router.post('/pagos/:id/anular', allowRoles('Administrador','Recepcionista'), async(req,res,next)=>{try{res.json(await appScriptPost('anularPago',{ID_Pago:req.params.id,Motivo:req.body.motivo||'Pago anulado por corrección',Usuario_Registro:req.user.sub}))}catch(e){next(e)}});
...
router.get('/registro', async (_req,res,next)=>{try{
  ...
  res.json(reservas.filter(x=>x.Estado==='Finalizada').map(x=>({id:x.ID_Reserva,nombre:gh[x.ID_Huesped]||'',habitacion:rh[x.ID_Habitacion]?.Numero||'',hospedaje:hp[rh[x.ID_Habitacion]?.ID_Hospedaje]||'',entrada:x.Fecha_Ingreso_Real||x.Fecha_Entrada,horaEntrada:x.Hora_Ingreso||'',salida:x.Fecha_Salida_Real||x.Fecha_Salida,horaSalida:x.Hora_Salida||'',personas:n(x.Cantidad_Personas),total:n(x.Pagado||x.Total),estado:x.Estado})));
}catch(e){next(e)}});
